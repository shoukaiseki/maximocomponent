...
![image](https://raw.githubusercontent.com/shoukaiseki/maximocomponent/master/maximo%E5%A4%9A%E5%9B%BE%E7%89%87imglibuser/image/001.png)


component-registry.xml 添加以下组件信息

```Xml
	<component-descriptor name="imglibuserview" instance-class="org.shoukaiseki.webclient.components.ImgLibUserComponents">
		<property-list>
			<!--滚动条 值例如 overflow:scroll; overflow-x:hidden;
			     使用滚动条需要将heigth值后面有px(例如:240px)-->
			<property name="scroll" />
			<!--单个图片显示宽度-->
			<property name="imgwidth" />
			<!--单个图片显示高度 无效果,根据图片高度自动-->
			<property name="imgheight" />
			<!--横向图片数量-->
			<property name="imgx"/>

			<property name="jsp-filename">
				<default-value>imglibuserview</default-value>
			</property>
			<property name="id">
				<flag name="final" />
			</property>
...
control-registry.xml 添加以下组件信息

```Xml

	<control-descriptor name="imglibuserview">
		<property-list>
			<property name="scroll" >
			...
			<property name="imgwidth">
				<default-value>120px</default-value>
			</property>
			<property name="imgheight">
				<default-value>120px</default-value>
			</property>
		...
		<component-list>
			<components id="co" layout="vertical">
				<components id="co3" layout="horizontal">
					<imglibuserview id="ri" scroll="@{scroll}" cssclass="@{cssclass}" imgwidth="@{imgwidth}"  imgheight="@{imgheight}"  imgx="@{imgx}"  height="@{height}" width="@{width}" datasrc="@{datasrc}"  label="@{label}" />
				</components>
			</components>
		</component-list>
		<containers>
...
maximouiweb/webclient/components 目录 增加 imglibuserview.jsp
